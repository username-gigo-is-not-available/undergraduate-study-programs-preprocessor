...
from utils.data import merge_dataframes
...
async def read_data(executor: ThreadPoolExecutor, input_data_file_paths: dict[str, Path]) -> \
...
def get_input_data_file_paths() -> dict[str, Path]:
    return {
        "study_programs": Path(ENVIRONMENT_VARIABLES.get('STUDY_PROGRAMS_INPUT_DATA_FILE_PATH')),
        "curricula": Path(ENVIRONMENT_VARIABLES.get('CURRICULA_INPUT_DATA_FILE_PATH')),
        "courses": Path(ENVIRONMENT_VARIABLES.get('COURSE_INPUT_DATA_FILE_PATH')),
    }


def prepare_data_for_saving(df_study_programs: pd.DataFrame,
                            df_curricula: pd.DataFrame,
                            df_courses: pd.DataFrame) -> list[dict[str, Path | pd.DataFrame]]:
    return [
        {
            'file_name': Path(ENVIRONMENT_VARIABLES.get('STUDY_PROGRAMS_DATA_OUTPUT_FILE_NAME')),
            'data': df_study_programs
        },
        {
            'file_name': Path(ENVIRONMENT_VARIABLES.get('CURRICULA_DATA_OUTPUT_FILE_NAME')),
            'data': df_curricula
        },
        {
            'file_name': Path(ENVIRONMENT_VARIABLES.get('COURSE_DATA_OUTPUT_FILE_NAME')),
            'data': df_courses
        },
        {
            'file_name': Path(ENVIRONMENT_VARIABLES.get('MERGED_DATA_OUTPUT_FILE_NAME')),
            'data': merge_dataframes(df_curricula, df_courses)
        }
    ]
...
async def save_data_to_file(data: dict[str, str | pd.DataFrame], output_dir: str) -> None:
    ...
    data.to_csv(Path(output_dir, f"{file_name}.csv"), index=False)
