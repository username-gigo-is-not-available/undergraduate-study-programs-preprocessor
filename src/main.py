...
from src.pipeline.offers_requires_pipeline import offers_requires_pipeline
from src.pipeline.professor_teaches_pipeline import professor_teaches_pipeline
...
def main() -> None:
    ...
    professor_teaches_pipeline(df_courses).build().run()
    df_study_programs: pd.DataFrame = study_programs_pipeline().build().run()
    offers_requires_pipeline(df_study_programs, df_courses).build().run()
